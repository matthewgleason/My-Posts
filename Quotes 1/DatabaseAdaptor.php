...
// Complete these four straightfoward functions and run as a CLI application
    ...
    public function addQuote($quote, $author) {
        ...
        $stmt = $this->DB->prepare("INSERT INTO quotations (added, quote, author, rating, flagged) VALUES (NOW(),  :bind_name1, :bind_name2 , '0', '0')");
        $stmt->bindParam(':bind_name1', $quote);
        $stmt->bindParam(':bind_name2', $author);
        $stmt->execute();
    }
    
    public function addUser($accountname, $psw){
        // TODO 4: Complete this function
        $stmt = $this->DB->prepare("INSERT INTO users (username, password) VALUES (:bind_name1, :bind_name2)");
        $stmt->bindParam(':bind_name1', $accountname);
        $stmt->bindParam(':bind_name2', $psw);
        $stmt->execute();
    ...
    public function deleteQuote($id) {
        ...
        $stmt->bindParam(':bind_name', $id);
        $stmt->execute();
    }
    
    public function incRating($id) {
        $stmt = $this->DB->prepare("UPDATE quotations SET rating = rating + 1 WHERE id = :bind_name");
        $stmt->bindParam(":bind_name", $id);
        $stmt->execute();
    }
    
    public function decRating($id) {
        $stmt = $this->DB->prepare("UPDATE quotations SET rating = rating - 1 WHERE id = :bind_name");
        $stmt->bindParam(':bind_name', $id);
        $stmt->execute();
    }
...
// Test table quotes
$theDBA->addQuote('Walking no water and developing software from a specification are esay if both are frozen.', 'Edward Berard');
